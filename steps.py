...
def upload_csv():
    ...
    if uploaded_file is not None:
        ...
        st.write(df)
        check_data_types(df)
        st.sidebar.title("Let's start analyzing")
        option = st.sidebar.selectbox("Select an option", [ "Visualize Data", "Preprocess", "ML Model"])
        if option == 'Visualize Data':
            gph.visualize_data(df)
        elif option == 'Preprocess':
            preprocess.preprocess(df)
        elif option == 'ML Model':
            reg.ml_model_reg(df)
            cls.ml_model_cls(df)
